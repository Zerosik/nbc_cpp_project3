...
int main() {
	...
	while (true) {
		drawLine();
		cout << "1. 아이템 추가, 2. 아이템삭제, 3. 정렬, 4.조회, 5.크기변경 0. 종료" << endl;
		int select;	
		string name;
		int num;//입력값처리용
		cin >> select;
		...
		case 1:
			...
			cout << "가격 : ";
			cin >> num;
			inven->addItem(BaseItem(0, name, num));
			break;
		case 2:
	...
	//cout << "오퍼레이터 사용한 복사 인벤토리 생성" << endl;
	//Inventory<BaseItem> assignWithOperator = inven;
	//assigned.assign(assignWithOperator);
	//assigned.printAllItems();
	//cout << "복사 인벤토리 아이템 수 : " << assignWithOperator.getSize() << ", 대입 아이템 수 : " << assignWithOperator.getCapasity() << endl;
